...
- **Nested-category repos** (vercel-labs / mattpocock style `skills/<lang>/<skill>`):
  on a flat-only backend (Claude) two skills sharing a leaf collapse onto one dir. The
  skill still works; its leaf collides with a sibling. Caught by the flat-emit lint.
...
No counts are given for those shapes — none are present in the local schools to measure.
